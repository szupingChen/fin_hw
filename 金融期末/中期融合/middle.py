...
class LateFusionPredictor(nn.Module):
    def __init__(self, news_dim, market_dim):
        ...
        self.news_attention = nn.Sequential(
            nn.Linear(self.hidden_dim * 2, 1),  # *2是因為雙向LSTM
            nn.Tanh()
        )
    ...
    def create_sequence_data(self, x, seq_len):
        # 檢查輸入維度
        if len(x.shape) != 2:
            raise ValueError(f"Expected 2D input, got {len(x.shape)}D")
        
        batch_size = x.size(0)
        if batch_size < seq_len:
            # 如果數據量小於序列長度，則填充
            padding = x[0].unsqueeze(0).repeat(seq_len - batch_size, 1)
    ...
    def forward(self, news_x, market_x):
        try:
            ...
            seq_len = news_x.size(1)  # 已經是序列數據，形狀應該是 [batch, seq_len, features]
            
            # 新聞特徵提取 - 處理每個時間步
            news_encoded = []
            for t in range(seq_len):
                news_t = self.news_encoder(news_x[:, t, :])
                news_encoded.append(news_t)
            news_encoded = torch.stack(news_encoded, dim=1)  # [batch, seq_len, hidden_dim]
            
            # 市場特徵提取 - 處理每個時間步
            market_encoded = []
            for t in range(seq_len):
                market_t = self.market_encoder(market_x[:, t, :])
                market_encoded.append(market_t)
            market_encoded = torch.stack(market_encoded, dim=1)  # [batch, seq_len, hidden_dim]
            
            # LSTM處理
            news_lstm_out, _ = self.news_lstm(news_encoded)  # [batch, seq_len, hidden_dim*2]
            market_lstm_out, _ = self.market_lstm(market_encoded)  # [batch, seq_len, hidden_dim*2]
            
            # 注意力機制 - 新聞
            news_attention = self.news_attention(news_lstm_out)  # [batch, seq_len, 1]
            news_attention = F.softmax(news_attention, dim=1)
            news_features = torch.sum(news_lstm_out * news_attention, dim=1)  # [batch, hidden_dim*2]
            
            # 注意力機制 - 市場
            market_attention = self.market_attention(market_lstm_out)  # [batch, seq_len, 1]
            market_attention = F.softmax(market_attention, dim=1)
            market_features = torch.sum(market_lstm_out * market_attention, dim=1)  # [batch, hidden_dim*2]
            
            # 獨立預測
            news_pred = self.news_predictor(news_features)  # [batch, 1]
            market_pred = self.market_predictor(market_features)  # [batch, 1]
            
            # 特徵融合
            combined_features = torch.cat([news_features, market_features], dim=1)  # [batch, hidden_dim*4]
            fusion_pred = self.fusion(combined_features)  # [batch, 1]
            
            return news_pred, market_pred, fusion_pred
            ...
            if 'news_lstm_out' in locals():
                print(f"LSTM輸出 - 新聞: {news_lstm_out.shape}")
            if 'market_lstm_out' in locals():
                print(f"LSTM輸出 - 市場: {market_lstm_out.shape}")
            if 'news_features' in locals():
                print(f"最終特徵 - 新聞: {news_features.shape}")
            if 'market_features' in locals():
                print(f"最終特徵 - 市場: {market_features.shape}")
            raise e
...
def late_fusion_loss(outputs, targets, model):
    ...
    if len(fusion_pred) > 1:
        direction_loss = -torch.mean(torch.sign(fusion_pred[1:] - fusion_pred[:-1]) * 
