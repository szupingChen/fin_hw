...
class LateFusionPredictor(nn.Module):
    def __init__(self, news_dim, market_dim):
        super().__init__()
        self.hidden_dim = 64     # 減小隱藏層維度
        self.num_heads = 8       # 減少注意力頭數
        self.seq_len = 5
        self.dropout_rate = 0.3  # 統一的dropout率
        ...
        self.cross_modal_norm = nn.LayerNorm(self.hidden_dim)
        ...
        self.self_attention_norm = nn.LayerNorm(self.hidden_dim)
        
        # 特徵融合網絡
        fusion_input_dim = self.hidden_dim * 4
        fusion_hidden_dim = self.hidden_dim * 2
        ...
        self.fusion_network = nn.ModuleList([
            nn.Sequential(
                ...
                nn.GELU(),
                nn.Dropout(self.dropout_rate),
                nn.Linear(fusion_hidden_dim, fusion_input_dim)
            ),
            nn.Sequential(
                nn.Linear(fusion_input_dim, fusion_hidden_dim),
                nn.LayerNorm(fusion_hidden_dim),
        ...
        # 殘差連接的線性變換
        self.news_residual = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim // 2),
            nn.LayerNorm(self.hidden_dim // 2),
            nn.Dropout(self.dropout_rate),
            nn.Linear(self.hidden_dim // 2, 1)
        )
        self.market_residual = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim // 2),
            nn.LayerNorm(self.hidden_dim // 2),
            nn.Dropout(self.dropout_rate),
            nn.Linear(self.hidden_dim // 2, 1)
    ...
    def add_positional_encoding(self, x):
        ...
        pos_emb = torch.zeros(batch_size, seq_len, dim, device=x.device)
    ...
    def forward(self, news_x, market_x):
        try:
            batch_size = news_x.size(0)
            seq_len = news_x.size(1)
            
            # 特徵編碼
            news_encoded = []
            market_encoded = []
            for t in range(seq_len):
                news_encoded.append(self.news_encoder(news_x[:, t, :]))
                market_encoded.append(self.market_encoder(market_x[:, t, :]))
            
            news_encoded = torch.stack(news_encoded, dim=1)
            market_encoded = torch.stack(market_encoded, dim=1)
            
            # 添加位置編碼
            news_encoded = self.add_positional_encoding(news_encoded)
            market_encoded = self.add_positional_encoding(market_encoded)
            
            # 多尺度時序注意力
            news_temps = []
            market_temps = []
            for i, (attention, norm) in enumerate(zip(self.temporal_attentions, self.temporal_norms)):
                # 新聞時序注意力
                news_temp, _ = attention(news_encoded, news_encoded, news_encoded)
                news_temp = news_encoded + norm(news_temp)  # 殘差連接
                news_temps.append(news_temp)
                
                # 市場時序注意力
                market_temp, _ = attention(market_encoded, market_encoded, market_encoded)
                market_temp = market_encoded + norm(market_temp)  # 殘差連接
                market_temps.append(market_temp)
            
            # 融合多尺度特徵
            news_temp = sum(news_temps) / len(news_temps)
            market_temp = sum(market_temps) / len(market_temps)
            
            # 跨模態注意力
            news_cross, _ = self.cross_modal_attention(news_temp, market_temp, market_temp)
            news_cross = news_temp + self.cross_modal_norm(news_cross)  # 殘差連接
            
            market_cross, _ = self.cross_modal_attention(market_temp, news_temp, news_temp)
            market_cross = market_temp + self.cross_modal_norm(market_cross)  # 殘差連接
            
            # 自注意力處理
            news_self, _ = self.self_attention(news_cross, news_cross, news_cross)
            news_self = news_cross + self.self_attention_norm(news_self)  # 殘差連接
            
            market_self, _ = self.self_attention(market_cross, market_cross, market_cross)
            market_self = market_cross + self.self_attention_norm(market_self)  # 殘差連接
            
            # 全局和局部特徵
            news_global = torch.mean(news_self, dim=1)
            market_global = torch.mean(market_self, dim=1)
            news_local = news_self[:, -1, :]
            market_local = market_self[:, -1, :]
            
            # 特徵融合
            combined_features = torch.cat([
                news_global, news_local,
                market_global, market_local
            ], dim=1)  # [batch_size, hidden_dim * 4]
            
            # 多層融合網絡
            fusion_layer1 = self.fusion_network[0](combined_features)  # [batch_size, hidden_dim * 4]
            fusion_layer1 = fusion_layer1 + combined_features  # 殘差連接
            
            fusion_layer2 = self.fusion_network[1](fusion_layer1)  # [batch_size, hidden_dim]
            
            # 預測
            main_pred = self.prediction_head(fusion_layer2)  # [batch_size, 1]
            
            # 分支預測（使用殘差連接）
            news_pred = self.news_residual(news_global) + 0.1 * main_pred
            market_pred = self.market_residual(market_global) + 0.1 * main_pred
            
            # 最終預測（加權融合）
            final_pred = main_pred + 0.05 * (news_pred + market_pred)
            
            return news_pred, market_pred, final_pred
            
        except RuntimeError as e:
            print(f"\n維度信息:")
            print(f"輸入 - 新聞: {news_x.shape}, 市場: {market_x.shape}")
            if 'news_encoded' in locals():
                print(f"編碼後 - 新聞: {news_encoded.shape}")
            if 'market_encoded' in locals():
                print(f"編碼後 - 市場: {market_encoded.shape}")
            raise e
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(X_news_train), batch_size):
            ...
            optimizer.zero_grad()
            news_pred, market_pred, fusion_pred = predictor(batch_news, batch_market)
        ...
        predictor.eval()
        with torch.no_grad():
            news_pred, market_pred, fusion_pred = predictor(X_news_test, X_market_test)
