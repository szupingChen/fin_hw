...
class LateFusionPredictor(nn.Module):
    ...
    def forward(self, news_x, market_x):
        try:
            ...
            # 堆疊時序特徵
            news_features = torch.stack(news_features, dim=1)
            market_features = torch.stack(market_features, dim=1)
            
            # 取最後一個時間步的特徵
            news_features = news_features[:, -1, :]
            market_features = market_features[:, -1, :]
            
            # 計算各分支預測
            news_pred = self.news_predictor(news_features)
            market_pred = self.market_predictor(market_features)
            
            # 使用固定權重(0.5)融合預測結果
            final_pred = 0.5 * news_pred + 0.5 * market_pred
            
            return news_pred, market_pred, final_pred, None  # None 代替原本的 modal_weights
